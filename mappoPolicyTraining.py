"""
	The file contains the MAPPO class to train with.
...
class MAPPO:
	"""
		This is the main MAPPO class we will use as our model in main.py
	"""
	def __init__(self, subpolicy,type, env, **hyperparameters):
		"""
			Initializes the MAPPO model, including hyperparameters.
		...
		# This is only used in case of secondary policy training where we start with the old updated network
		actor_model_old = 'ppo_actor_updatedmultiwalker_v7_primary.pth'
		critic_model_old = 'ppo_critic_updatedmultiwalker_v7_primary.pth'
		if self.discrete:
	...
	def learn(self, env_name,failure_observations):
		...
		while i_so_far < self.training_step:                                                                       # ALG STEP 2
			...
			batch_obs, batch_acts, batch_log_probs, batch_rtgs, batch_lens = self.rollout(failure_observations)      # ALG STEP 3
			...
			if i_so_far % self.save_freq == 0:
				if self.subpolicy:
					...
					if(self.type == 'secondary'):
						torch.save(self.actor.state_dict(), './ppo_actor_policy'+env_name+'_safe'+'.pth')
						torch.save(self.critic.state_dict(), './ppo_critic_policy'+env_name+'_safe'+'.pth')
				else:
	...
	"""Function to calculate beta as per time step beta = reward/(penalty-t)
	"""

	def calculate_beta(self,rew,penalty,ep_t):
		beta = abs(rew/(penalty-ep_t))
		return beta/beta
			...
			for ep_t in range(self.max_timesteps_per_episode):
				...
				if self.subpolicy and  self.type == 'primary':
					if env_name == 'multiwalker_v7':
						...
						if spec_type == 'local':
							#Calculates the hull angle of the individual agents. Add only if spec is negative
							if((0.7 - obs[0])<0):
								penalty_local = (0.7 - obs[0])
								penalty =  penalty_local*0.1
						# Penalty from distance
						#If there is a left neighbour then penalise for crash with left neighbour
						if spec_type == 'mutual':
							if(obs[24] != 0):
								if((obs[24]*-1 - 0.24)<0):
									penalty_mutual = (obs[24]*-1 - 0.24)
									penalty =  penalty_mutual*0.1
									
							#If there is  right neighbour then penalise for crash with right neighbour
							if(obs[26] != 0):
								if((obs[26] - 0.24)<0):
									penalty_mutual = (obs[26] - 0.24)
									penalty =  penalty_mutual*0.1
									
						#Penalty from package angle
						if spec_type == 'global':
							if((0.25 - obs[30])<0):
								penalty_global = (0.25 - obs[30])
								penalty =  penalty_global*0.1
								
						if(penalty < 0):
							beta = self.calculate_beta(rew,penalty,ep_t)
						# print(penalty)
						rew = rew + penalty * beta
...
def test(env, actor_model, is_discrete):
	...
	eval_policy(policy=policy, env=env, render=False, is_discrete=is_discrete)
