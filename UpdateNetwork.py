	...
	while i_so_far < num_of_iterations:
		...
		if(len(batch_lens)!=0):
			...
			for _ in range(n_updates_per_iteration):
				...
				V, curr_log_probs = evaluate(critic_old, policy_old, batch_obs, batch_acts)
		...
		torch.save(policy_old.state_dict(), './ppo_actor_updated'+env_name+'_primary.pth')
		torch.save(critic_old.state_dict(), './ppo_critic_updated'+env_name+'_primary.pth')
...
def get_trajectory(policy_new,env,fail_file):
	...
	while t < timesteps_per_batch:
		...
		for j in range(len(failure_observations[index][3])):
			...
			env.env.env.env.walkers[failure_observations[index][3][j][1]].hull.angle = failure_observations[index][3][j][0]
...
def correct_policy(env, actor_model_old, critic_model_old,actor_model_new, critic_model_new,is_discrete,file_name):
	...
	# Load in the old and the sub-actor model saved by the PPO algorithm
	...
	# Update the old policy to a new policy via gradient updates using the subpolicy
