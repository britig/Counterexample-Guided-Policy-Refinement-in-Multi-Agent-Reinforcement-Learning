"""
	...
	training in main.py with mappo.py.
"""
...
def display(observation,policy,env,is_discrete,render):
	#name = env.unwrapped.spec.id
	ep_ret = 0
	#this is specific to inverted pendulum environment
	env.env.env.env.state = observation
	obs = observation
	#obs = observation
	#print(f'obs ============= {obs}')
	traj = [obs]
	...
	while not done:
		...
		env.step(action)
		#print(f'obs Left neighbor relative X Right neighbor relative X============= {obs[24]}===={obs[26]}')
		#print(f'Hull angle 0 and 1============= {obs[0]}')
		obs, rew, done, _ = env.last()
		#print(f'done x ============= {done}')
		'''if(obs[0] > 2 or env.env.env.env.walkers[0].hull.angle < -0.8):
			print(f'hull angle ============= {env.env.env.env.walkers[0].hull.angle}')'''
		traj.append(obs)
